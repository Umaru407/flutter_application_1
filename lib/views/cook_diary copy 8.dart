...
class CookListScreen extends ConsumerStatefulWidget {
  ...
  const CookListScreen({required this.meal}); //
  @override
...
class _CookListState extends ConsumerState<CookListScreen> {
  ...
  Future<void> _retrieveDiaries() async {
  ...
  @override
  Widget build(BuildContext context) {
    List<CookDiaryModel> mealDiaries = [];
    final selectDate = ref.watch(selectedDateProvider);

    switch (widget.meal) {
      case '早餐':
        mealDiaries = breakfastDiaries;
        break;
      case '午餐':
        mealDiaries = lunchDiaries;
        break;
      case '晚餐':
        mealDiaries = dinnerDiaries;
        break;
      default:
    }
    // print("999");
